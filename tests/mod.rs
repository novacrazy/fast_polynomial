...
fn test_all_at_once_rational() {
    ...
    let c_den = [1.00, 4.4990248, 3.299489, 0.300803];

    #[allow(clippy::items_after_statements)]
    const NUM_XS: usize = 4;
    let all_xs: [_; NUM_XS] = [-0.5, 0.1, 0.5, 0.9];
    let all_xs_wrapped = ArrayWrap::new(all_xs);
    for j in 1..=c_den.len() {
        for i in 0..=c_num.len() {
            let from_one_at_a_time: [_; NUM_XS] =
                core::array::from_fn(|x_idx| rational(all_xs[x_idx], &c_num[..i], &c_den[..j]));
            let from_all_at_once =
                rational::<ArrayWrap<NUM_XS, f64>, f64>(all_xs_wrapped, &c_num[..i], &c_den[..j]);
            for x_idx in 0..NUM_XS {
                assert_feq!(1e-10, from_one_at_a_time[x_idx], from_all_at_once[x_idx]);
            }
        }
